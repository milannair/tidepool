...
use tidepool_common::index::recall::{brute_force, measure_recall};
...
fn bench_bruteforce_vs_hnsw(c: &mut Criterion) {
    let dims = 128;
    ...
    c.bench_function("bruteforce_search", |b| {
        b.iter(|| {
            let _ = brute_force(&query, &vectors, 10, DistanceMetric::Cosine);
        })
    });

    c.bench_function("hnsw_vs_bruteforce_recall", |b| {
        b.iter(|| {
...
criterion_group!(benches, bench_hnsw_search, bench_bruteforce_vs_hnsw);
